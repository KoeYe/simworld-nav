...
REAL_SIGNALS = Path("/data/murray/paris_signals_kerb/citycore-paris")


@pytest.mark.skipif(not (REAL_SIGNALS / "signal_visibility.json").exists(),
                    reason="the Paris signal album is not present here")
class TestTheShippedSidecarWasMeasuredByThisCode:
    def test_the_sidecar_names_the_method_it_used(self):
        ...
        # so a sidecar written by the old flip test cannot pass as one written
        # by this code. Both lamps, each in its own bake.
        assert "per-phase" in data["method"]
        assert "red lamp" in data["method"] and "green lamp" in data["method"]
        assert data["static_red_bigger_than_lamp"] <= data["legible_count"]
        assert 0 < data["legible_count"] <= data["approaches"]
