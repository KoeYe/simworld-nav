"""The point-navigation chain, tested against the rules PLAN.md 9 states.
...
# ─────────────────────────────────────────────────────────────────────────────
# The pose lattice (PLAN.md 9.5)
# ─────────────────────────────────────────────────────────────────────────────


class TestPoseLattice:
    def test_quantisation_is_shared_not_cached_only(self):
        """PLAN.md 9.5 forbids snapping in cached mode alone.
...
# ─────────────────────────────────────────────────────────────────────────────
# Ground-plane depth (PLAN.md 9.3)
# ─────────────────────────────────────────────────────────────────────────────
...
class TestGroundPlaneDepth:
    ...
    def test_horizon_is_no_ground_hit_not_a_huge_number(self):
        """PLAN.md 9.4 wants no_ground_hit distinct from a wall hit. Returning a
    ...
    def test_provider_declares_itself_geometric(self):
        """PLAN.md 9.3 requires results from this provider to be reported as
...
# ─────────────────────────────────────────────────────────────────────────────
# The chain end to end (PLAN.md 9.4)
# ─────────────────────────────────────────────────────────────────────────────
...
class TestResolvePointAction:
    ...
    def test_2d_depth_without_a_provider_is_refused(self):
        ...
        differently-named nav_point_3d and void PLAN.md 9.7.6's comparison."""
    ...
    def test_strict_variant_refuses_rather_than_repairing(self):
        """PLAN.md 9.2's strict variant treats the model's distance as
    ...
    def test_the_source_point_is_kept_verbatim(self):
        """PLAN.md 9.4 wants the raw model output auditable after the chain."""
...
class TestDistanceError:
    def test_error_is_logged_even_when_snapping_repairs_it(self):
        """PLAN.md 9.2: without this, a policy emitting a constant distance looks
...
class TestDecidePointNavigation:
    ...
    def test_node_keyed_album_cannot_serve_point_modes(self):
        """PLAN.md 9.5: a point action ends at a lattice pose, and a node-keyed
...
class TestEnvSpecCarriesPointNav:
    def test_a_usable_map_declares_all_three_modes(self):
        ...
        # nav_waypoint stays first: PLAN.md 9 makes it the production path and a
...
class TestParisPointNavIsCoherent:
    ...
    def test_paris_lattice_is_finer_than_its_edges(self):
        ...
        action lands, which is why PLAN.md 9.5 rejects snapping to nodes."""
        decision = decide_point_navigation(self.PARIS_ANALYSIS)
