...
PLAN.md M1 accepts when "a fixed existing DeliveryBench action trace runs
...
def test_step_budget_exhaustion_truncates_rather_than_terminates():
    """PLAN.md 7: budget exhaustion is truncation, not task termination."""
    instance = _episode_spec().model_copy(update={"seed": SEED})
...
def test_events_are_deterministic_and_unique():
    """PLAN.md 12.6: events must not duplicate, and must replay identically."""
    def collect():
        instance = _episode_spec().model_copy(update={"seed": SEED})
        runtime = VagenTextRuntime(map_name=MAP, max_steps=20)
        try:
