...
class TestWhatTheMapDraws:
    ...
    def test_it_works_with_no_route_and_no_destination(self, paris):
        """Opening the app without asking for anything is still a map."""
        env = self.env(paris)
...
class TestTheMapReachesTheCourier:
    ...
    def test_asking_the_phone_puts_a_map_on_the_screen(self, paris):
        session = self.session(paris)
        assert not any(f.kind == "map" for f in session.observe().frames)
