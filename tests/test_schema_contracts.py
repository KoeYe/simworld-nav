"""Contract tests for every protocol schema (PLAN.md M1).
...
def test_all_schemas_are_v0_before_m12(schema_id, model):
    """PLAN.md M1: all schemas remain v0.x with no compatibility promise."""
...
def test_differing_minor_is_rejected_while_v0(schema_id, model):
    """PLAN.md M1: early consumers pin exact schema revisions."""
    payload = model.to_dict()
...
# ─────────────────────────────────────────────────────────────────────────────
# terminated vs truncated: separately tested causes (PLAN.md M1)
# ─────────────────────────────────────────────────────────────────────────────
...
def test_infrastructure_failure_is_a_truncation_not_a_task_failure():
    """PLAN.md 12.2 records evaluator infrastructure timeouts separately."""
...
# ─────────────────────────────────────────────────────────────────────────────
# Geometry: units are explicit (PLAN.md 6.1 P1)
# ─────────────────────────────────────────────────────────────────────────────
...
def test_neighbors_ignore_excluded_edges_by_default():
    """PLAN.md M2: excluded edges are unavailable to episode generation."""
    graph = valid_fixtures()["embodiedbench/nav_graph"]
