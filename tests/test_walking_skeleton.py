...
PLAN.md M1 accepts when "the walking-skeleton command exits zero from a clean
...
def test_trajectory_links_to_the_environment_it_ran_in(tmp_path):
    ...
    # PLAN.md 12.1: the environment hash is the benchmark's "base commit".
    assert spec.environment_sha256 == world.content_hash()


def test_score_report_declines_to_invent_the_primary_metric(tmp_path):
    """PLAN.md 12.5 defines it against an upper-bound policy that does not exist yet."""
    code, _ = _run_skeleton(tmp_path)
    assert code == 0
...
def test_generated_episode_spec_is_reproducible_across_processes():
    """PLAN.md M4 in miniature: same (world, config, seed) -> identical spec."""
