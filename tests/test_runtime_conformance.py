...
PLAN.md 7.2: for a seeded action sequence the two runtimes must match on action
...
PLAN.md schedules this at M5, but the cached runtime exists at R1 because the
...
def test_full_trace_conformance(tmp_path):
    """The PLAN.md 7.2 comparison, run over a seeded action sequence."""
    instance, text, cached = _pair(tmp_path)
    try:
        text.reset(instance)
        cached.reset(instance)
...
def test_images_appear_on_every_turn(tmp_path):
    """PLAN.md 11.1's gate needs images on multiple environment turns."""
    instance, text, cached = _pair(tmp_path)
    try:
