"""The task plugin protocol (PLAN.md 8.1).

A task may read a ``WorldBundle`` and call a ``Runtime``, but must never import
UE or cached-runtime implementation code (PLAN.md 4.1). Nothing in this module
...
class TaskRequirements:
    """What a task needs a world to provide before it can run (PLAN.md 6.1 P4)."""
...
class SolvabilityVerdict:
    """PLAN.md 8.3: episode generation is followed by an oracle feasibility check."""
