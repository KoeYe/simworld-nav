"""Composable Delivery configuration (PLAN.md 8.2).

PLAN.md 8.2 replaces "a single large mutable config" with validated composition:
...
complete. PLAN.md 8.3 requires generation to be followed by a feasibility check;
the profiles make most infeasibility detectable before generation even runs.

Every field that changes an outcome is declared observable. PLAN.md 17 lists
...
class RewardProfile:
    ...
    anyone. No model judges anything. PLAN.md 11.3 keeps training shaping
...
class DeliveryTaskConfig:
    """The composed configuration (PLAN.md 8.2)."""
    ...
    def observable_summary(self, courier: CourierProfile | None = None) -> dict[str, Any]:
        """Exactly what the agent is told at reset (PLAN.md 8.2, 10.1)."""
