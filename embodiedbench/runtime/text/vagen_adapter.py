...
PLAN.md M1: "Wrap the existing VAGEN text env without changing its behavior",
...
PLAN.md 7 requires the distinction. That separation is derived from the
...
class VagenTextRuntime:
    """The text runtime (PLAN.md 7.1) backed by the vendored engine."""
    ...
    def __init__(
        ...
        *,
        ...
        self.capabilities = RuntimeCapabilities(
            ...
            # PLAN.md M6 requires text and cached runtimes to advertise
    ...
    def _mark_candidates(self) -> list[MarkCandidate]:
        """Reachable neighbours as numbered Set-of-Marks candidates (PLAN.md 9.1)."""
        dm = self._dm()
        if dm is None:
            return []
        try:
    ...
    def _events_for_step(self, info: dict[str, Any], error: str | None) -> list[Event]:
        ...
        identity, so PLAN.md 12.6's "retrying a step cannot duplicate events"
    ...
    def _termination(
        ...
        PLAN.md 7 requires the distinction and PLAN.md M1 requires separately
    ...
    def authoritative_state(self) -> dict[str, Any]:
        """The state roots PLAN.md 7.2 compares across runtimes."""
        if self._env is None:
            return {}
