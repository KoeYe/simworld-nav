"""The common runtime API (PLAN.md 7, Contract B).
...
``capabilities.supports_snapshot``, not a mandatory method — PLAN.md 7 makes it
...
``authoritative_state_digest`` is not in PLAN.md's sketch but is required to
implement PLAN.md 7.2: conformance compares "task state and inventory, economy
...
class StepIndexMismatch(RuntimeError_):
    ...
    PLAN.md 7.3 requires a monotonic step index and an idempotency key in the
...
class EmbodiedRuntime(Protocol):
    ...
    def authoritative_state_digest(self) -> str:
        """Canonical digest of the state PLAN.md 7.2 compares across runtimes."""
        ...


@runtime_checkable
class SnapshotCapable(Protocol):
    """Optional extension (PLAN.md 7). Evaluator/harness-only in v1."""
