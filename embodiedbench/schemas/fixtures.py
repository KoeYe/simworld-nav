"""Canonical valid and invalid fixtures for every contract (PLAN.md M1).
...
that *must* be rejected, paired with the PLAN.md rule it would otherwise
...
def _nav_graph() -> NavGraph:
    ...
    edges = [
        ...
        NavEdge(
            ...
            exclusion_reason="unvalidated 45.3 m connector (PLAN.md 3.2); may cross non-road space",
        ),
    ]
...
def invalid_fixtures() -> list[InvalidFixture]:
    """Payloads that must fail, each tied to the PLAN.md rule it breaks."""
    graph = _nav_graph()
    step = _step_result()
    ...
    return [
        InvalidFixture(
            ...
            "PLAN.md 6.1 P2 / 17: synthetic links are unsafe until a NavMesh path confirms them",
        ),
        InvalidFixture(
            ...
            "PLAN.md M2: every excluded edge must record why",
        ),
        InvalidFixture(
            ...
            "PLAN.md 5.1: stable IDs with referential integrity",
        ),
        InvalidFixture(
            ...
            "PLAN.md 6.1 P3: VLM proposals persist with confidence for review",
        ),
        InvalidFixture(
            "absolute_path_in_bundle",
            WorldBundle,
            _mutate(_world_bundle(), previews=["/home/murray/previews/topdown.webp"]),
            "PLAN.md 5.1: no absolute machine-specific paths in portable artifacts",
        ),
        InvalidFixture(
            "escaping_relative_path",
            WorldBundle,
            _mutate(_world_bundle(), previews=["../../etc/passwd"]),
            "PLAN.md 5.1: bundle paths must stay inside the bundle",
        ),
        InvalidFixture(
        ...
        InvalidFixture(
            ...
            "PLAN.md 9.3: range comes from the external depth provider, not the model",
        ),
        InvalidFixture(
            ...
            "PLAN.md 9.1: a mark resolves to a node before the action is recorded",
        ),
        InvalidFixture(
            "unknown_action_type",
            ActionEnvelope,
            _mutate(_action_envelope(), action={"type": "teleport", "target_node": "int_2"}),
            "PLAN.md 10.2: actions are schema-validated",
        ),
        InvalidFixture(
            ...
            "PLAN.md 2.1 / 11.1 / M6: observation and tool tokens are excluded from policy loss",
        ),
        InvalidFixture(
            ...
            "PLAN.md 12.6: retrying a step cannot duplicate reward or events",
        ),
        InvalidFixture(
            "total_reward_disagrees_with_turns",
            Trajectory,
            _mutate(trajectory, total_reward=99.0),
            "PLAN.md 11.1: total reward is preserved exactly once",
        ),
        InvalidFixture(
            "courier_hides_an_outcome_relevant_extra",
            CourierProfile,
            _mutate(_courier(), extra={"secret_speed_bonus": 1.25}),
            "PLAN.md 17: hidden courier attributes make instances unfair and uninterpretable",
        ),
        InvalidFixture(
            ...
            "PLAN.md 12.6: divergent policies must observe the same order schedule",
        ),
        InvalidFixture(
            ...
            "PLAN.md 9.6: ControllerResult reports the final pose it reached",
        ),
        InvalidFixture(
            ...
            "PLAN.md 9.4: point modes must declare max_range_m",
        ),
        InvalidFixture(
            ...
            "PLAN.md 9.3: external depth determines range in nav_point_2d_depth",
        ),
    ]
...
def uncovered_schemas() -> set[str]:
    """Versioned envelopes with no valid fixture — must be empty (PLAN.md M1)."""
    registered = {
