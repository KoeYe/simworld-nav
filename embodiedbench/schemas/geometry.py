...
PLAN.md 6.1 P1 requires explicit UE, bundle-world, map-render, camera, and
...
The Paris export is authored in centimetres (PLAN.md 3.2 quotes bounds in cm)
while navigation ranges are specified in metres (PLAN.md 9.4 ``max_range_m``).
...
class FrameName(str, Enum):
    """The coordinate frames PLAN.md 6.1 P1 names."""
...
class Pose(SchemaModel):
    ...
    ground pose with a heading (PLAN.md 9.5's lattice is "2.0 m spatial with 8
    ...
    def close_to(self, other: "Pose", *, position_cm: float, yaw_deg: float) -> bool:
        ...
        PLAN.md M10 states conformance tolerances as "at most 5 cm and 0.5
...
class CameraIntrinsics(SchemaModel):
    """Pinhole intrinsics plus the depth convention (PLAN.md 6.1 P1, P5)."""
    ...
    # PLAN.md 9.3 forbids relative depth: the unprojection maths is only valid
    ...
    def unproject(self, u_norm: float, v_norm: float, distance_cm: float) -> Vec3:
        ...
        This is the shared step PLAN.md 9.4 puts at the head of both point
    ...
    def project(self, point: "Vec3") -> tuple[float, float, float] | None:
        ...
        The exact inverse of :meth:`unproject`, which is what makes PLAN.md
