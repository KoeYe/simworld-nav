...
**Unknown fields are rejected.** PLAN.md M1 requires unknown-field tests to
...
drift PLAN.md 7.2's conformance suite exists to catch. ``extra="forbid"``
...
differs raises ``VersionError``, because PLAN.md 5.4 defines major as "changed
...
**Errors are typed.** PLAN.md M1 requires invalid versions, coordinate frames,
...
class SchemaVersion:
    """A semantic version with PLAN.md 5.4's compatibility rule."""
    ...
    def can_read(self, payload: "SchemaVersion") -> bool:
        ...
        Major must match exactly. While the whole package is ``0.x``, PLAN.md M1
