"""Trajectory and score report (PLAN.md 10.4, 12.5).

PLAN.md 10.4 lists what every turn must record. Two of those items are the ones
that quietly invalidate an RL run if they are wrong, so they are validated
rather than trusted:

**The loss mask must exclude environment and tool observations.** PLAN.md 2.1
calls observation-token loss masking a correctness risk, PLAN.md 11.1 makes it a
trainer selection gate, and PLAN.md M6 requires that observation, tool, and
...
**Reward must be counted once.** PLAN.md 11.1 requires that context compaction
and fan-out preserve total reward exactly once, and PLAN.md 12.6 requires that
...
class CostAccounting(SchemaModel):
    """PLAN.md 10.4: token, latency, service, and environment cost."""
...
class Trajectory(SchemaModel):
    """A full episode record, trainer-neutral (PLAN.md 11)."""
...
class ScoreReport(SchemaModel):
    """An evaluator's verdict (PLAN.md 8.1 evaluate(), 12.5)."""
    ...
    success: bool = False
    # PLAN.md 12.5's primary Delivery metric. Optional because PointNav does not
    # have one, and because the metric is undefined when the bound is
    # non-positive -- a case PLAN.md requires defined behaviour for, which here
    ...
    # PLAN.md 11.3: training shaping is reported separately and never used for
