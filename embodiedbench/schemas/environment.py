"""Task ``OverlaySpec`` and derived ``EnvironmentBundle`` (PLAN.md 5.2, 5.3).

An overlay is a declarative patch, never an edited copy of the source map. Two
of PLAN.md 5.2's requirements are enforced structurally: every placement records
...
``EnvironmentBundle`` declares compatibility rather than implying it (PLAN.md
5.3): a certificate is keyed by ``(task plugin, embodiment profile, navigation
mode, runtime)``, matching PLAN.md 6.2's "not a single vague map grade".
"""
...
class NavigationMode(str, Enum):
    """PLAN.md 9's three high-level navigation modes."""
...
class PointExecutionVariant(str, Enum):
    """PLAN.md 9.2's two declared execution variants."""
...
class OverlayPlacement(SchemaModel):
    """One materialized task asset, with the provenance PLAN.md 5.2 requires."""

    placement_id: StableId
    affordance: str = Field(min_length=1)
    position: Vec3
    yaw_deg: float = 0.0
    # "Reuse valid authored assets where possible" (PLAN.md 5.2.1) means a
    ...
    def _reuse_or_spawn(self) -> "OverlayPlacement":
        ...
        if spawned and not self.data_layer:
            # PLAN.md 5.2.3: spawn or modify only through a named Data Layer.
            raise ValueError(
...
class RemovalManifest(SchemaModel):
    """PLAN.md 5.2.6: an inverse manifest so the source project is unchanged."""
...
class OverlaySpec(SchemaModel):
    """A declarative task patch over a base world (PLAN.md 5.2)."""
    ...
    def deficit(self, available: dict[str, int]) -> dict[str, int]:
        """How many of each affordance are still missing (PLAN.md 6.1 P4)."""
        out = {}
        for affordance, requirement in self.requires_affordances.items():
...
class ObservationManifestEntry(SchemaModel):
    """One baked observation row (PLAN.md 6.1 P5)."""
...
class Certificate(SchemaModel):
    """A grade for one exact combination, not for a map (PLAN.md 6.2)."""
    ...
    def _grade_matches_evidence(self) -> "Certificate":
        ...
        if self.grade is CertificationGrade.B and not self.limitations:
            # PLAN.md 6.2: grade B means "known limitations are declared". A
...
class SensorSpec(SchemaModel):
    """What a runtime can observe here (PLAN.md 7.1 'declared sensor capability')."""
    ...
    def _lattice_is_all_or_nothing(self) -> "SensorSpec":
        # PLAN.md 9.5 versions lattice resolution and headings in
...
class EnvironmentBundle(SchemaModel):
    """Base world + overlay + sensors + certificates (PLAN.md 5.3)."""
    ...
    def _declares_rather_than_implies(self) -> "EnvironmentBundle":
        ...
        # A point mode needs a pose lattice in every runtime, because PLAN.md
        ...
        if point_modes & set(self.supported_navigation_modes):
            if self.sensors.pose_lattice_spacing_cm is None:
                raise ValueError(
                    ...
                    "(PLAN.md 9.5); otherwise cached and live runtimes would differ"
                )
