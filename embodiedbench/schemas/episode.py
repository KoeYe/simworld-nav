"""``EpisodeSpec`` and benchmark instances (PLAN.md 8.2, 12.2).

PLAN.md 12.2 is explicit that "a seed alone is not a benchmark instance": an
...
PLAN.md 8.2 requires every profile field that changes outcomes to be visible to
...
class Budgets(SchemaModel):
    ...
    Wall-clock is deliberately absent: PLAN.md 12.2 makes it a reported cost
...
class CourierProfile(SchemaModel):
    """Task-side courier configuration (PLAN.md 8.2), distinct from embodiment."""
    ...
    # reset (PLAN.md 8.2, and PLAN.md 17's "hidden courier attributes" risk).
    ...
    def _no_hidden_bonuses(self) -> "CourierProfile":
        ...
        # bonus of exactly the kind PLAN.md 17 forbids.
...
class ScheduledOrder(SchemaModel):
    """One pinned order in the frozen stream (PLAN.md 12.2)."""
...
class EpisodeSpec(SchemaModel):
    """A frozen, reproducible episode (PLAN.md 8.1 generate(), 12.2)."""
