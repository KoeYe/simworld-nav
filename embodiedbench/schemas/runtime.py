...
PLAN.md 7 fixes the shape. Three things are load-bearing and enforced here.

**``terminated`` and ``truncated`` are separate, with separate causes.**
PLAN.md 7 asks for Gymnasium semantics "including the distinction between task
termination and budget truncation", and PLAN.md M1 requires them to have
...
``privileged_state_ref`` (PLAN.md 7), and ``Observation`` has no field that can
hold it. PLAN.md 12.6 and 13 both make leak tests a release gate, so the type
system is the first line rather than the only one.

**Action outcomes are typed.** PLAN.md 9.4 enumerates the outcomes a point
...
class RuntimeMode(str, Enum):
    """PLAN.md 7.1: there are three runtime modes, not two."""
...
class NavWaypointAction(SchemaModel):
    """PLAN.md 9.1, the production path for DeliveryBench v1."""
    ...
    # resolves to a stable node id *before recording the action* (PLAN.md 9.1).
...
class NavPoint3DAction(SchemaModel):
    """PLAN.md 9.2: the model predicts the image point *and* the metric range."""
...
class NavPoint2DDepthAction(SchemaModel):
    """PLAN.md 9.3: the model picks where; an external depth provider picks how far."""
    ...
    def _no_model_distance(self) -> "NavPoint2DDepthAction":
        if self.target.distance_m is not None:
            ...
            # nav_point_3d and invalidate the comparison PLAN.md 9.7.6 wants.
            raise ValueError(
...
NavigationAction = Annotated[
    Union[NavWaypointAction, NavPoint3DAction, NavPoint2DDepthAction],
    Field(discriminator="type"),
]
AnyAction = Annotated[
    Union[NavWaypointAction, NavPoint3DAction, NavPoint2DDepthAction, TaskAction],
    Field(discriminator="type"),
]
...
class ActionEnvelope(SchemaModel):
    ...
    ``subgoal`` exists for interpretability, and PLAN.md 10.2 requires that task
...
class ActionStatus(str, Enum):
    """PLAN.md 7: accepted / rejected / failed, plus controller status."""
...
class ActionResult(SchemaModel):
    ...
    def _failures_explain_themselves(self) -> "ActionResult":
        if self.status is not ActionStatus.ACCEPTED and not self.error_code:
            # PLAN.md M6 requires every invalid action to receive typed feedback
...
class MediaRef(SchemaModel):
    """A reference to an image, by relative path and content hash (PLAN.md 10.4)."""
...
class Event(SchemaModel):
    """A structured environment event (PLAN.md 7, 13.2)."""
    ...
    # PLAN.md 12.6 requires leak tests; marking an event privileged lets the
...
class TerminationReason(str, Enum):
    ...
    PLAN.md M1 requires terminated and truncated to have separately tested
...
class RuntimeCapabilities(SchemaModel):
    """What a runtime advertises (PLAN.md 7)."""
    ...
    # PLAN.md 7: an optional extension, evaluator/harness-only in every v1
