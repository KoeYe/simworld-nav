...
PLAN.md 5.1's required properties are enforced here rather than documented:
...
``certified`` or ``excluded`` with no third state, since PLAN.md M2 requires
...
NavMesh evidence, because PLAN.md 3.3 and 17 both single out the two long Paris
...
def _check_relative_path(value: str) -> str:
    ...
    PLAN.md 5.1 requires no absolute machine-specific paths in portable
...
# A path inside a bundle. PLAN.md 5.1: no absolute machine-specific paths.
...
class LabelProvenance(str, Enum):
    """Where a semantic label came from (PLAN.md 5.1, 6.1 P3)."""
...
class EdgeStatus(str, Enum):
    """PLAN.md M2: every edge is certified or excluded, with no third state."""
...
class NodeKind(str, Enum):
    """PLAN.md 3.2 counts these separately, so the schema keeps them distinct."""
...
class NavMeshEvidence(SchemaModel):
    """Proof that a NavMesh path exists along an edge (PLAN.md M2)."""
        ...
        if self.status is EdgeStatus.CERTIFIED:
            if self.provenance is EdgeProvenance.SYNTHETIC_CONNECTOR:
                ...
                if evidence is None or not evidence.path_found:
                    # PLAN.md 6.1 P2: synthetic links are unsafe until a NavMesh
                    # path confirms them; PLAN.md 17: they must not leak into eval.
                    raise ValueError(
...
class NavGraph(SchemaModel):
    ...
    def certified_road_coverage(self) -> float:
        """Fraction of total edge length that is certified (PLAN.md M2, ADR-0004)."""
...
class SemanticEntity(SchemaModel):
    ...
    def _vlm_labels_carry_confidence(self) -> "SemanticEntity":
        # PLAN.md 6.1 P3: VLM proposals are persisted with confidence for review.
...
class SourceProvenance(SchemaModel):
    """What the bundle was compiled from (PLAN.md 5.1, 6.1 P0)."""
...
class WorldBundle(SchemaModel):
    """The immutable base map artifact (PLAN.md 5.1)."""
