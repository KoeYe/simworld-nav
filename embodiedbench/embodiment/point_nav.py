"""Point-based navigation: the shared geometry chain (PLAN.md 9.2-9.5).
...
do need geometry, and PLAN.md 9.4 fixes the order:
...
PLAN.md 9.5 is emphatic that quantisation is part of the *action semantics*, not
...
On the depth provider, PLAN.md 9.3 says plainly that on flat ground with a known
...
# PLAN.md 9.4: start the Paris pilot near 18 m, about its median graph edge, so
# one point action and one waypoint action cover comparable ground.
DEFAULT_MAX_RANGE_M = 18.0
# PLAN.md 9.5's pilot design point.
...
class PoseLattice:
    ...
    PLAN.md 9.5: a continuous end pose has no cached observation, so either
...
class DepthProvider(Protocol):
    """Supplies metric range for an image point (PLAN.md 9.3)."""
...
class GroundPlaneDepth:
    ...
    PLAN.md 9.3 sanctions this and simultaneously warns about what it means: on
    ...
    ``no_ground_hit`` -- distinct, as PLAN.md 9.4 requires, from hitting a wall.
...
class ModelPredictedDepth:
    """The model's own ``distance_m`` (PLAN.md 9.2)."""
...
class PointNavResolution:
    """A resolved point action, with the typed outcome PLAN.md 9.4 requires.
...
def resolve_point_action(
    *,
    ...
    """Run PLAN.md 9.4's chain, raising a typed rejection at the first failure."""
    ...
    # 2. range cap. PLAN.md 9.2's strict variant treats the distance as
    ...
    # 5. quantise, in every runtime alike (PLAN.md 9.5)
    ...
    return PointNavResolution(
        ...
        # PLAN.md 9.7.6 is meaningless if a repaired distance looks accepted.
        outcome=(
...
def distance_error_m(model_distance_m: float, external_distance_m: float) -> float:
    """PLAN.md 9.2: always log |model - external|, even when snapping repairs it.
