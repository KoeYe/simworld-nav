"""Overlay compiler: extend a map to meet a task's requirements (PLAN.md 5.2, 6.1 P4).
...
This closes that gap the way PLAN.md 5.2 requires: as *a declarative patch, not
...
The sequence follows PLAN.md 6.1 P4: read the requirements, produce a deficit
...
DERIVED_ROOT = Path("/data/murray/derived_maps")
...
def report_deficit(
    ...
    *,
    ...
    """PLAN.md 6.1 P4: a deficit report before anything is modified."""
...
def compile_overlay(
    ...
    *,
    ...
    # PLAN.md 5.2.1: reuse valid authored assets where possible. The overlay
...
def source_unchanged(map_name: str, before: dict[str, str], maps_root: Path | None = None) -> bool:
    ...
    PLAN.md M3 requires the source manifest hashes to be unchanged after an
