...
def parse_chunk(reply: str, allowed: set[str], max_calls: int = 1) -> list[Call]:
    ...
    """
    if max_calls <= 1:
        return [parse_reply(reply, allowed)]
    ...
    for name, raw_args in calls:
        tool, args, kwargs = build_call(name, raw_args, allowed)
...
class ChunkedCourierSession(CourierSession):
    ...
    def _chunked_step(self, reply: str) -> ChunkTurnLog:
        ...
        try:
            calls = parse_chunk(reply, set(self.allowed), self.action_chunk)
        ...
        turn.tool_kind = TOOLS_BY_NAME[calls[0].tool].kind.value
        ...
        self.run.turns.append(turn)
        self.feedback = self._chunk_feedback(rows, messages, refused)
