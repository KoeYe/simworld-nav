...
PLAN.md 10 separates model access from agent behaviour. This class is the model
...
Parse failures are ordinary events, not errors. PLAN.md 10.2 requires the raw
...
class Qwen3VLPolicy:
    ...
    def act(self, observation: Observation, runtime: Any, step_index: int) -> ActionEnvelope | None:
        ...
        if observation.last_action_result is not None and observation.last_action_result.message:
            # PLAN.md M6: an invalid action gets typed feedback on the next turn.
